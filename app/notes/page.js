...
export default function NotesPage() {
  ...
  const [pdfList, setPdfList] = useState([]); // List of available PDFs
  const [selectedPdf, setSelectedPdf] = useState(null); // Selected PDF file name
  const [pageNumber, setPageNumber] = useState(1);
  const [numPages, setNumPages] = useState(null);
  const [scale, setScale] = useState(1.0); // Scale for zoom functionality

  // Fetch list of PDFs and load notes from local storage
  useEffect(() => {
  ...
  // Delete a specific note and update local storage
